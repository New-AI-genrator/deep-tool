...
// Find the position to insert new blog posts (before the closing bracket of the array and helper functions)
const insertPosition = fileContent.lastIndexOf('  }'); // Find the last blog post closing
const helperFunctionsPosition = fileContent.indexOf('\n// Helper functions');
...
const beforeInsert = fileContent.substring(0, insertPosition + 3);
const afterInsert = fileContent.substring(insertPosition + 3, helperFunctionsPosition);
const helperFunctions = fileContent.substring(helperFunctionsPosition);

// Combine everything correctly
const updatedContent = beforeInsert + ',\n' + newBlogPostsString + afterInsert + helperFunctions;
